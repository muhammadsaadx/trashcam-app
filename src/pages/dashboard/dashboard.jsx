...
const Dashboard = () => {
  ...
  return (
    <div style={styles.dashboard}>
      <header style={styles.dashboardHeader}><h3>Dashboard</h3></header>
      <div style={styles.dashboardContent}>
        <div style={styles.chartRow}>
          <div style={styles.chartCard}>
            <div style={styles.chartHeader}>
              <h2 style={styles.headerText}>Monthly Litter Count</h2>
              <select value={selectedYear} onChange={e => setSelectedYear(+e.target.value)} style={styles.dropdown}>
                {Object.keys(litterRateData).map(year => <option key={year} value={year}>{year}</option>)}
              </select>
            </div>
            <div style={styles.chartWrapper}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={litterRateData[selectedYear] || []} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="count" stroke="#07715B" activeDot={{ r: 8 }} strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
